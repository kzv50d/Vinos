...
if 'pred_final' not in st.session_state:
    st.session_state['pred_final'] = classifier.predict(df)[0]
...
if st.sidebar.button("Cargar Vino Excelente Garantizado 🍷"):
    ...
    st.session_state['pred_final'] = classifier.predict(vino_premiado)[0]

st.subheader('Predicción')
 
if st.session_state['pred_final'] == 0:
    st.error('El modelo predice: **Baja Calidad** 🍇')
elif st.session_state['pred_final'] == 1:
